...
output_file = "results/final_track_pseudo.csv"


def evaluate_prediction(
    start_frame, end_frame, vertices, gt_track_3d, idx, mask, object_motions_valid
):
    track_errors = []
    for frame_idx in range(start_frame, end_frame):
        ...
        # current_motion_valid = object_motions_valid[frame_idx - 1][idx][new_mask]
        # pred_x = pred_x[current_motion_valid]
        # gt_track_points = gt_track_points[current_motion_valid]
        if len(pred_x) == 0:
            track_error = 0
        else:
...
for dir_name in dir_names:
    ...
    kdtree = KDTree(object_points[0])
    dis, idx = kdtree.query(gt_track_3d[0][mask])

    train_track_error = evaluate_prediction(
        1, train_frame, object_points, gt_track_3d, idx, mask, object_motions_valid
    )
