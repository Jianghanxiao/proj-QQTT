...
with open("data_config.csv", newline="", encoding="utf-8") as csvfile:
    ...
    for row in reader:
        ...
        for i in range(3):
            ...
            # Prepare the high-resolution image. If it's already generated, then copy it
            if i == 0 and shape_prior.lower() == "true":
                os.system(
                    f"cp {base_path}/{case_name}/shape/high_resolution.png {output_path}/{case_name}/{i}_high.png"
                )
            else:
                os.system(
        ...
        if shape_prior.lower() == "true":
            os.system(
                f"cp {base_path}/{case_name}/shape/object.glb {output_path}/{case_name}/shape_prior.glb"
            )
