...
def get_spring_mass(pcd):
    ...
    for i in range(len(vertices)):
        [k, idx, _] = pcd_tree.search_hybrid_vector_3d(points[i], 0.05, 11)
        idx = idx[1:]
        for j in idx:
...
def get_spring_mass_visual(vertices, springs, rest_legnths):
    ...
    line_colors = [np.array([0., 1., 0.]) * np.abs((1 - np.linalg.norm(vertices[springs[i][0]] - vertices[springs[i][1]]) / rest_legnths[i])) for i in range(len(springs))]
...
def demo1():
    ...
    vertices, springs, rest_lengths = get_spring_mass(table)
