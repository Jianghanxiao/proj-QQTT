...
def render_sets(dataset : ModelParams, iteration : int, pipeline : PipelineParams, skip_train : bool, skip_test : bool, separate_sh: bool, remove_gaussians: bool = False, exp_name: str = "expA_to_expB", transfer_gs: bool = False):
    with torch.no_grad():
        ...
        output_path = './output_dynamic_out_domain'

        bg_color = [1,1,1] if dataset.white_background else [0, 0, 0]
        background = torch.tensor(bg_color, dtype=torch.float32, device="cuda")

        # example name: single_lift_cloth_3_to_single_clift_cloth_3
        source_name = exp_name.split("_to_")[0]
        ...
        # rollout
        ctrl_pts_path = os.path.join('./experiments_out_domain', exp_name, "inference.pkl")
...
def transfer_gaussians(gaussians_src, source_name, target_name):
    ...
    ctrl_pts_tgt_path = os.path.join('./experiments_out_domain', source_name + '_to_' + target_name, "inference.pkl")
...
if __name__ == "__main__":
    ...
    with open('./rendering_finished_dynamic_out_domain.txt', 'a') as f:
