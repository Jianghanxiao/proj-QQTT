...
def interpolate_motions(bones, motions, relations, xyz, rot=None, quat=None, weights=None, device='cuda'):
    ...
    # Compute the transformed particles (OOM)
    ...
    xyz_transformed = torch.zeros(n_particles, 3).to(device)
    for i in range(n_bones):
        transformed = (xyz - bones[i]) @ bone_transforms[i, :3, :3].T + bone_transforms[i, :3, 3] + bones[i]
        xyz_transformed += transformed * weights[:, i, None]
    ...
    if quat is not None:
        ...
        # quats = (base_quats[None] * weights[:, :, None]).sum(dim=1)  # (n_particles, 4) OOM
        quats = torch.zeros(n_particles, 4, device=device)
        for i in range(n_bones):
            quats += base_quats[i] * weights[:, i, None]
...
def interpolate_motions_feng(bones, motions, relations, xyz, rot=None, quat=None, weights=None, device='cuda', step='n/a'):
    # bones: (n_bones, 3)
    # motions: (n_bones, 3)
    # relations: (n_bones, k)
    # indices: (n_bones,)
...
def interpolate_motions_feng_speedup(bones, motions, relations, xyz, rot=None, quat=None, weights=None, weights_indices=None, device='cuda', step='n/a'):
